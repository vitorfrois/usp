...
//
// Para executar em v�rios nodes do cluster, rodar, por exemplo:
//  mpirun -np 1 --hostfile halley.txt binary
// a funcao MPI_Comm_spawn() pode indicar o hostfile tambem.
//
//
// A aplica��o funciona com -np maiores que 1. Permite exemplificar que se trata de uma primitiva coletiva 
// e que apenas um processo cria os processos filhos.
// to run: mpirun -np 8 01-spawn-simple
//
// o numero de processos a gerar � limitado pelo MPI em fun��o do nr de slots.
// Os slots determinam o nr de processos a escalonar em cada processador
// o padr�o para o slot � um processo por core, mas pode mudar.
//
...
//
// -host namehost1:slots,namehost2:slots,namehost3:slots (ele trava se n�o colocar o hostfile no spawn)
//
// 
// para o MPI_Comm_spawn executar sobre diferentes n�s, deve-se executar o mpirun com --hostfile ou --host
// isso permite registrar mais slots � execu��o que ser�o utilizados no spawn
