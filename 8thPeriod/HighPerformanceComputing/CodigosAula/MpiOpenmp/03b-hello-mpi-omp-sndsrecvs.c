...
int  main(int argc, char *argv[])  
{
	...
	if ( myrank == 0) 
	{
		#pragma omp parallel num_threads(NT) private(total_threads, thread_number, msgtag)
		{
			...
			// com rank > 0. Por isso src varia de 1 até npes-1 (i.e., src<npes).
	...
	else 
	{
	                             ...
	                             num_threads(NT)
		{
		    ...
		    printf("Worker on %s, process %d/%d, thread %d/%d, local_i=%d\n", 
			processor_name, myrank, npes, thread_number, total_threads, local_i);
			sprintf(bufsend, "Worker:%d on %s, thread %d/%d, local_i=%d", myrank, processor_name, thread_number, total_threads, local_i);
//		    printf("%s \n", bufsend);
		    fflush(0);
	
			// cada thread T de cada processo com rank > 0 envia uma mensagem para a 
			// respectiva thread T do processo com myrank == 0. Isso eh feito
