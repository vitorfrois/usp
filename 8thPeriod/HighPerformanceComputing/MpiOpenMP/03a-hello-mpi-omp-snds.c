...
int  main(int argc, char *argv[])  
{
	...
	//MPI_Init(&argc, &argv);
	MPI_Init_thread(&argc, &argv, /*MPI_THREAD_FUNNELED, MPI_THREAD_SINGLE */ MPI_THREAD_MULTIPLE, &provided);
	...
	else 
	{
	                             ...
	                             num_threads(NT)
		{
		    ...
		    printf("Worker: thread %d/%d, from process %d/%d on %s, local_i = %d\n", 
			thread_number, total_threads, myrank, npes, processor_name, local_i);
		    msgtag = 1;
			sprintf(bufsend, "Worker %d, thread %d, local_i=%d", myrank, thread_number, local_i);
//		    printf("%s \n", bufsend);
		    fflush(0);
