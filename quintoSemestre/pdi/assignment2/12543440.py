...
class Filter:
    ...
    @classmethod
    def normalize(cls, image: np.array):
        image_min = image.min()
        image_max = image.max()
    ...
    def apply_filter(cls):
        ...
        cls.new_image = cls.normalize(new_image) * 255
...
class Gaussian(Filter):
    ...
    @classmethod
    def read_pass_input(cls, parameters_list: list):
        cls.omega_r2 = int(parameters_list[0])
        cls.omega_c2 = int(parameters_list[1])
        cls.set_omega()

    @classmethod
    def set_omega(cls):
        cls.omega_r2 = np.power(cls.omega_r2, 2)
        cls.omega_c2 = np.power(cls.omega_c2, 2)
...
class Butterworth(Filter):
    ...
    def read_band_input(cls, parameters_list: list):
        cls.D0 = int(parameters_list[0])
        cls.D1 = int(parameters_list[1])
        cls.n0 = float(parameters_list[2])
    ...
    def low_pass_function(cls, a, b, d=None, n=None):
        ...
        if n is None:
            n = cls.n0
        den = (1 + np.power((cls.diff_sum_sqrt(a, b) / d), 2 * n))
        return 1 / den
